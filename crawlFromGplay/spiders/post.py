...
class PostdemoSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for i in range(len(self.url)):
            ...
            # print(data)
            # for (url, pack) in zip(self.start_urls, self.package):
            yield scrapy.FormRequest(self.start_urls[0], headers=headers, formdata=data, callback=self.parse, meta={'data': self.package[i]} )
    ...
    def parse(self, response):
        ...
        yield item
        # item['Authority'] = response.text()
        # item['Item_name'] = re.xpath('//h1[@class="AHFaub"]/span/text()').extract_first()
