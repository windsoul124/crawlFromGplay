...
class GplaySpider(scrapy.Spider):
    ...
    # start_urls = urls
    start_urls = {'https://play.google.com/store/apps/details?id=pro.cryptotab.android',
                  }
    # 爬取规则
    # rules = (
    ...
    def parse(self, response):
        ...
        for title in titles:
            ...
            item['androidVersionText'] = title.xpath('//div[contains(text(),"Requires Android")]/following-sibling::span/div/span/text()').extract_first()
            item['androidVersion'] = title.xpath('//div[contains(text(),"Requires Android")]/following-sibling::span/div/span/text()').extract_first().replace(' and up', '')
            ...
            item['minInstalls'] = title.xpath('//div[contains(text(),"Installs")]/following-sibling::span/div/span/text()').extract_first().replace(',', '').replace('+', '')
            item['released'] = title.xpath('//div[contains(text(),"Updated")]/following-sibling::span/div/span/text()').extract_first()
            item['version'] = title.xpath('//div[contains(text(),"Version")]/following-sibling::span/div/span/text()').extract_first()
            item['privacyPolicy'] = title.xpath('//span[@class="htlgb"]/div/a[contains(text(), "Privacy Policy")]/@href').extract_first()
            ...
            item['containsAds'] = title.xpath('//*[@id="fcxH9b"]/div[4]/c-wiz/div/div[2]/div/div/main/c-wiz[1]/c-wiz[1]/div/div[2]/div/div[1]/div[2]/text()').extract_first()
            ...
            # item['ratings'] = title.xpath('//span[@class="EymY4b"]/span[2]/text()').extract_first()
            yield item
